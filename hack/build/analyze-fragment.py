...
def load_kconfig(srctree: Path, arch: str) -> kconfiglib.Kconfig:
    os.environ.update(
        ...
        HOSTCXX="g++",
        CC_VERSION_TEXT="gcc",
        # Kconfig probes shell out to these (scripts/Kconfig.include). Anything
        # left unset probes as missing, and symbols gated on it silently
        # resolve to n -- CONFIG_PAHOLE_VERSION reads 0 without PAHOLE, which
        # drags DEBUG_INFO_BTF down with it. Mirror the kernel Makefile's
        # defaults so the probes agree with a real build.
        PAHOLE="pahole",
        NM="nm",
        OBJCOPY="objcopy",
        OBJDUMP="objdump",
...
def requested_value(raw: str) -> str:
    """Normalise a config-file value for comparison against `Symbol.str_value`.

    kconfiglib reports string symbols unquoted, so `CONFIG_X="fbdev"` has to
    lose its quotes before it can be compared to `fbdev`.
    """
    value = raw.strip()
...
def unmet_in(sym: kconfiglib.Symbol) -> list[str]:
    """Symbols inside sym's dependency that currently evaluate to n.

    A hint at *why* a dependency is false, not a proof: only meaningful when
    the dependency as a whole already evaluates to n.
    """
    return sorted(
        {
            item.name
            ...
            and item.name
            and item is not sym
            and item.str_value == "n"
        }
    )


def explain_ineffective(sym: kconfiglib.Symbol, want: str, got: str) -> str:
    """Why this line did not produce the value it asked for."""
    if not is_assignable(sym):
        return "no prompt; value comes from select/default only"
    if kconfiglib.expr_value(sym.direct_dep) == 0:
        blockers = unmet_in(sym)
        listed = ", ".join("CONFIG_" + b for b in blockers[:4])
        more = f", +{len(blockers) - 4} more" if len(blockers) > 4 else ""
        return f"unmet dependency ({listed}{more})" if blockers else "unmet dependency"
    if got != "n" and want == "n":
        selectors = sorted(
            s.name
            for s in sym.kconfig.unique_defined_syms
            if any(t[0] is sym for t in s.selects) and s.str_value != "n"
        )
        if selectors:
            return "forced on by " + ", ".join("CONFIG_" + s for s in selectors[:4])
    return "value overridden during resolution"
...
def main() -> int:
    ...
    # What each line asked for versus what the symbol actually ends up with.
    # Computed before apply_disables() below, which deliberately overwrites
    # values and would invalidate the comparison.
    ineffective = []
    for name in order:
        sym = kconf.syms.get(name)
        if not sym or not sym.nodes:
            continue  # unknown; reported above
        if not is_assignable(sym):
            continue  # inert; reported under "Entries with no effect"
        want, got = requested_value(values[name]), sym.str_value
        if want != got:
            ineffective.append((name, want, got, explain_ineffective(sym, want, got)))

    section(f"Entries that do not take effect ({len(ineffective)})")
    if not ineffective:
        print("None. Every line produces the value it asks for.")
    else:
        print("These lines ask for one value and the kernel resolves another. The")
        print("config reads as though the option is set; the built kernel disagrees.\n")
        for name, want, got, why in ineffective:
            print(f"  CONFIG_{name}: asked {want!r}, got {got!r}")
            print(f"    {why}")
    ...
    # 2a. Lines that cannot do anything, because the symbol has no prompt.
    # Covers enables as well as disables: CONFIG_DRM_TTM=y is just as inert as
    ...
    section(f"Entries with no effect ({len(inert)})")
    if not inert:
        print("None. Every symbol named here is settable from a config file.")
    else:
        print(
